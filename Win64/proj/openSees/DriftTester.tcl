...
proc DriftTester {numStories DriftLimit FloorNodes  h1 htyp} {
 global CollapseFlag;
 global Result;
 set CollapseFlag "NO"
 ...
 set MAXDrift $DriftLimit
	...
	for { set h 0 } { $h <= $numStories-1} {incr h} {
		...
		if { $TDrift > $MAXDrift } {
			...
			set fileID [open $Result/CollapseState.txt w];   # Create/Open CollapseState.txt file (writing permission)
