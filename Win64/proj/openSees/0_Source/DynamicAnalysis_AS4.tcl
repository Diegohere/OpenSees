...
proc DynamicAnalysis {dt dt_anal_Step GMtime numStories DriftLimit FloorNodes h1 htyp} {
...
set nr_iter_1 100;
set nr_iter_2 200;
set nr_analyse 20; # 1/20/50/100 This is the number of steps to do in a current solver

set main_test NormDispIncr; # RelativeNormDispIncr/NormDispIncr/EnergyIncr/RelativeEnergyIncr
# set alternative_test EnergyIncr; # RelativeNormDispIncr/NormDispIncr/EnergyIncr/RelativeEnergyIncr

constraints Transformation; # Plain/Transformation (Lu)
numberer RCM;
system SparseSYM; # UmfPack/SparseSYM/Cusp
test $main_test $tol_0 $nr_iter_2 $show_iter
algorithm KrylovNewton

integrator Newmark 0.55 0.2765625; # Newmark 0.50 0.25/Newmark 0.55 0.2765625 (Lu)/GeneralizedAlpha 1.0 0.6
analysis VariableTransient

# ------- SET VARIABLES BEFORE ENTERING THE LOOP --------------------------------------------------------------------------------------------------------------------------------------------

set dt_analysis $dt_anal_Step; # timestep of analysis
set dt_anal_min1 [expr $dt_anal_Step/12]; # minimum time step for Variable Transient Analysis
...
set dt_anal_max [expr $dt_anal_Step*2]; # maximum time step for Variable Transient Analysis

set nr_iter1 10; # Number of maximum iterations for Variable Transient Analysis
