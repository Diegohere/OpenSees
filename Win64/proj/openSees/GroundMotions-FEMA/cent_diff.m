...
function [Amax] =  cent_diff(GMFolderName,period, deltat, xi, GMotion, g)

 mass = 1.0;
 
 %cd (GMFolderName)
% open file to read earthquake data
infile =  load(GMotion);
cd ..

%% Read the ground motion file and put everything in a column vector

L  = length(infile(:,1)); CL = length(infile(1,:));

for i=1:L
    p((1+(i-1)*CL):(CL+(i-1)*CL),1) = infile(i,:)'*g;
end
