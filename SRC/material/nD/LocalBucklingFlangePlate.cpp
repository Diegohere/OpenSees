...
static int numLocalBucklingFlangePlate = 0;


void* OPS_LocalBucklingFlangePlate(void) {
	if (numLocalBucklingFlangePlate == 0) {
		opserr << "Using the LocalBucklingFlangePlate material" << endln;
		numLocalBucklingFlangePlate++;
	}
	...
	if (OPS_GetIntInput(&nInputsToRead, materialTag) != 0) {
		opserr << "WARNING invalid nDMaterial LocalBucklingFlangePlate tag" << endln;
		return 0;
	}
...
LocalBucklingFlangePlate::LocalBucklingFlangePlate(int tag, double E, double poissonRatio,
	...
	double alphaReg)
	: NDMaterial(tag, ND_TAG_LocalBucklingFlangePlate),
	elasticModulus(E),
...
LocalBucklingFlangePlate::LocalBucklingFlangePlate()
	: NDMaterial(0, ND_TAG_LocalBucklingFlangePlate),
	elasticModulus(0.),
	poissonRatio(0.),
	initialYield(0.),
	qInf(0.),
...
*/
int LocalBucklingFlangePlate::timeIntegration() {
			...
			if (phiCompression / (2. / 3. * pow(initialYield, 2)) <= RETURN_MAP_TOL) { //loading is elastic
				...
				if (abs(strainPostBucklingTrial(0)) < SMALL_NUMBER) {
					...
					if ((3. / 2. * (2. / 3. * pow(stressTrial(0), 2) + 2. * pow(stressTrial(1), 2) + 2. * pow(stressTrial(2), 2)) - pow(sigmaCTrial, 2)) / pow(sigmaC0Stress, 2) <= SMALL_NUMBER) { // not yet at capping point
						...
						// Check if capping point is reached
						//if (3. / 2. * (2. / 3. * pow(stressTrial(0), 2) + 2. * pow(stressTrial(1), 2) + 2. * pow(stressTrial(2), 2)) - pow(sigmaCTrial, 2) >= -SMALL_NUMBER) { // capping point is reached
						//double testValue = (3. / 2. * (2. / 3. * pow(stressTrial(0), 2) + 2. * pow(stressTrial(1), 2) + 2. * pow(stressTrial(2), 2)) - pow(sigmaCTrial, 2)) / pow(sigmaC0Stress, 2);
						if ((3. / 2. * (2. / 3. * pow(stressTrial(0), 2) + 2. * pow(stressTrial(1), 2) + 2. * pow(stressTrial(2), 2)) - pow(sigmaCTrial, 2)) / pow(sigmaC0Stress, 2) >= -SMALL_NUMBER) { // capping point is reached							cappingPoint = 1;
							cappingPoint = 1;
							strainPostBucklingTrial(0) = -SMALL_NUMBER;
							deltaStrain_trial = deltaStrain_todo + deltaStrain_converged4Peak;
						}
...
* @return 0 if successful
*/
int LocalBucklingFlangePlate::returnMappingHardening(Vector strain_nPlus1, Vector alphaTot, Vector etaTrial, int switchUVCRecovUVCPoint) {
	// Initialize all the variables
	int retVal = 0;
	...
	if (iterationNumber_ReturnMapping >= MAXIMUM_ITERATIONS_RETURNMAPPING && fabs(phiVM / (2. / 3. * pow(initialYield, 2))) > RETURN_MAP_TOL) {
		opserr << "LocalBucklingFlangePlate::returnMappingHardening return mapping hardening stage did not converge!" << endln;
...
* @return 0 if successful
*/
int LocalBucklingFlangePlate::returnMappingSoftening(Vector strain_nPlus1, Vector relativeStressTrial, Vector backstressTot, double yieldStressTot) {
	// Initialize all the variables
	int retVal = 0;
	bool convergedReturnMapping = false;
	unsigned int iterationNumber_ReturnMapping = 0;
	...
	if (iterationNumber_ReturnMapping >= MAXIMUM_ITERATIONS_RETURNMAPPING && fabs(phiComp / (2. / 3. * pow(initialYield, 2))) > RETURN_MAP_TOL) {
		opserr << "LocalBucklingFlangePlate::returnMappingSoftening return mapping softening stage did not converge!" << endln;
...
int LocalBucklingFlangePlate::returnMappingPlRecovStage(Vector strain_nPlus1) {
	// Initialize all the variables
	int retVal = 0;
	bool convergedReturnMapping = false;
	unsigned int iterationNumber_ReturnMapping = 0;
	...
	if (iterationNumber_ReturnMapping >= MAXIMUM_ITERATIONS_RETURNMAPPING && fabs(phiTens / (2. / 3. * pow(initialYield, 2))) > RETURN_MAP_TOL) {
		opserr << "LocalBucklingFlangePlate::returnMappingPlRecovStage return mapping plastic recovery stage did not converge!" << endln;
		opserr << "\tDelta epsilon 11 = " << strainTrial[0] - strainConverged[0] << endln;
		opserr << "\tDelta epsilon 12 = " << strainTrial[1] - strainConverged[1] << endln;
		opserr << "\tDelta epsilon 13 = " << strainTrial[2] - strainConverged[2] << endln;
...
int LocalBucklingFlangePlate::returnMappingUVCRecovStage(Vector strain_nPlus1, Vector alphaTot) {
	// Initialize all the variables
	int retVal = 0;
	bool convergedReturnMapping = false;
	unsigned int iterationNumber_ReturnMapping = 0;
	...
	// Warn the user if the algorithm did not convergein the return mapping for hardening and return -1
	if (iterationNumber_ReturnMapping >= MAXIMUM_ITERATIONS_RETURNMAPPING && fabs(phiVM / (2. / 3. * pow(initialYield, 2))) > RETURN_MAP_TOL) {
		opserr << "LocalBucklingFlangePlate::returnMappingUVCRecov return mapping UVC recovery stage did not converge!" << endln;
		opserr << "\tDelta epsilon 11 = " << strain_nPlus1[0] - strainConverged[0] << endln;
		opserr << "\tDelta epsilon 12 = " << strain_nPlus1[1] - strainConverged[1] << endln;
		opserr << "\tDelta epsilon 13 = " << strain_nPlus1[2] - strainConverged[2] << endln;
		opserr << "\tExiting with yield function = " << phiVM << " > " << RETURN_MAP_TOL * (2. / 3. * pow(initialYield, 2)) << endln;
...
void LocalBucklingFlangePlate::calculateConsistentTangentModulusElastic(double etaTangent) {
	//stiffnessTrial = elasticMatrix;
...
void LocalBucklingFlangePlate::calculateConsistentTangentModulusHardening(double consistParam_plastic, double fBar,
...
void LocalBucklingFlangePlate::calculateConsistentTangentModulusSoftening(const Vector& strain_nPlus1, const Vector& backstressTot,
...
void LocalBucklingFlangePlate::calculateConsistentTangentModulusPlRecovStage(Vector strain_nPlus1, double consistParam_plRecov, double yieldStressTot, Vector relativeStressNPlus1, Vector backstressTot) {
	// Initialize the variables
...
/* ----------------------------------------------------------------------------------------------------------------- */
void LocalBucklingFlangePlate::calculateConsistentTangentModulusUVCRecov(Vector strain_nPlus1, double consistParam_plastic, double fBar,
...
*/
int LocalBucklingFlangePlate::setTrialStrain(const Vector& v) {
...
*/
int LocalBucklingFlangePlate::setTrialStrain(const Vector& v, const Vector& r) {
...
* @return 0 if successful
*/
int LocalBucklingFlangePlate::setTrialStrainIncr(const Vector& v) {
...
* Note that this material model is rate independent.
*/
int LocalBucklingFlangePlate::setTrialStrainIncr(const Vector& v, const Vector& r) {
...
const Vector& LocalBucklingFlangePlate::getStrain() {
...
const Vector& LocalBucklingFlangePlate::getStress() {
...
const Matrix& LocalBucklingFlangePlate::getTangent() {
...
double LocalBucklingFlangePlate::getYieldStress() {
...
* @return 0 if successful
*/
int LocalBucklingFlangePlate::commitState() {
...
* @return 0 if successful
*/
int LocalBucklingFlangePlate::revertToLastCommit() {
...
* @return 0 if successful
*/
int LocalBucklingFlangePlate::revertToBeforeCapping(bool cappingPoint) {
	if (cappingPoint == 0)
	{
		strainPlasticTrial = strainPlasticConverged;
		strainPEqTrial = strainPEqConverged;
...
int LocalBucklingFlangePlate::revertToBeforeSwitchPlRecovUVC(bool switchPlRecovUVCPoint) {
...
int LocalBucklingFlangePlate::revertToBeforeSwitchUVCRecovUVC(bool switchUVCRecovUVCPoint) {
	if (switchUVCRecovUVCPoint == 0)
	{
		strainPlasticTrial = strainPlasticConverged;
		strainPEqTrial = strainPEqConverged;
...
* @return 0 if successful
*/
int LocalBucklingFlangePlate::revertToStart() {
...
*/
NDMaterial* LocalBucklingFlangePlate::getCopy() {

	LocalBucklingFlangePlate* theCopy;
	theCopy = new LocalBucklingFlangePlate(this->getTag(), elasticModulus, poissonRatio,
...
*/
NDMaterial* LocalBucklingFlangePlate::getCopy(const char* code) {
	if (strcmp(code, getType()) == 0) {
		...
		opserr << "LocalBucklingFlangePlate::getCopy invalid NDMaterial type, expecting " << code << endln;
		return 0;
	}
}
...
* @return 0 if successful
*/
int LocalBucklingFlangePlate::sendSelf(int commitTag, Channel& theChannel) {
	// Throw error to let the user know that paralleliziation not yet implemented
	opserr << "Fatal: Paralleliziation for LocalBucklingFlangePlate is not implemented yet!" << endln;
	return -1;
}
...
* @return 0 if successful
*/
int LocalBucklingFlangePlate::recvSelf(int commitTag, Channel& theChannel,
...
(see OPS_Globals.h)
*/
void LocalBucklingFlangePlate::Print(OPS_Stream& s, int flag) {

	// if (flag == OPS_PRINT_PRINTMODEL_MATERIAL) {
	if (flag == 2) {
		s << "LocalBucklingFlangePlate tag: " << this->getTag() << endln;
		s << "   E: " << elasticModulus << " ";
		s << "  fy: " << initialYield << " ";
		s << "   Qinf: " << qInf << " ";
		s << "   b: " << bIso << " ";
	...
	if (flag == 25000) {
		s << "\t\t\t{";
		s << "\"name\": \"" << this->getTag() << "\", ";
		s << "\"type\": \"LocalBucklingFlangePlate\", ";
		s << "\"E\": " << elasticModulus << ", ";
		s << "\"fy\": " << initialYield << ", ";
		s << "\"Qinf\": " << qInf << ", ";
		s << "\"b\": " << bIso << ", ";
...
void LocalBucklingFlangePlate::calculateElasticStiffness() {
...
Matrix LocalBucklingFlangePlate::calculateComplianceMatrix() {
...
void LocalBucklingFlangePlate::initializeEigendecompositions() {
...
*
*/
double LocalBucklingFlangePlate::dotprod3(const Vector& v1, const Vector& v2) {
	double res = 0.;
	for (unsigned int i = 0; i < N_DIMS; ++i)
		res += v1(i) * v2(i);
	return res;
...
*
*/
Vector LocalBucklingFlangePlate::vecMult3(const Vector& v1, const Vector& v2) {
...
double LocalBucklingFlangePlate::calculateYieldStressPlastic() {
...
double LocalBucklingFlangePlate::calculateIsotropicModulus() {
...
*
*/
double LocalBucklingFlangePlate::calculateEk(unsigned int i) {
	return exp(-gammaK[i] * (strainPEqTrial - strainPEqConverged));
}

/* ----------------------------------------------------------------------------------------------------------------- */

double LocalBucklingFlangePlate::calculateChi1c() {
	double chi1c = 0.;
	double sigmaSurSigmaY = 0.;
...
double LocalBucklingFlangePlate::calculateSigmaSurSigmaY() {
	double sigmaSurSigmaY = 0.;
	double alphaAngle = 0.;
	double cPlate = 0.;
...
double LocalBucklingFlangePlate::calculateDSigmaSurSigmaYdEpsilonPB11() {
...
void LocalBucklingFlangePlate::initializeBChi1c() {
	b_chi1c = (pow(sigmaC0Stress, 2)) / ((pow(sigmaC0Stress, 2) - pow(sigmaDMStress, 2)) * pow(alpha_chi1c, 2));
}

/* ----------------------------------------------------------------------------------------------------------------- */

double LocalBucklingFlangePlate::calculateEtaTangentReduce() {
	double etaTangent = 0.;
...
Vector LocalBucklingFlangePlate::calculateDCdLambdaPB(double etaTangent, double dPhiCompdXiVector11) {
	Vector dCdLambdaPB = Vector(N_DIMS);
	dCdLambdaPB.Zero();
	double dEtaTangentdEpsiPb11 = 0.;
...
double LocalBucklingFlangePlate::calculateDEtaTangentdEpsiPb11() {
	double dEtaTangentdEpsiPb11 = 0.;
...
void LocalBucklingFlangePlate::setTensileEllipsoidYieldSurf(double yieldStressTot, Vector alphaTot, double targetStress4PLRecov) {
...
double LocalBucklingFlangePlate::calculateChi1t(double yieldStressTot, double alphaTot11) {
	double chi1t = 0.;
	double f1t = 0.;
...
double LocalBucklingFlangePlate::calculateF1t(double yieldStressTot, double alphaTot11) {
	double f1t = 0.;
	double tBezier = 0.;
	double sigmaBezier = 0.;
...
double LocalBucklingFlangePlate::calculateTBezier() {
	double tBezier = 0;
...
void LocalBucklingFlangePlate::calculateRatioAlphaBackstress() {
	double dSigmaBezierDEpsiPb11 = 0.;
...
void LocalBucklingFlangePlate::initializeErc() {
...
void LocalBucklingFlangePlate::computeSigmaCDegradation() {
...
/* ----------------------------------------------------------------------------------------------------------------- */
Vector LocalBucklingFlangePlate::computeBackstressTotPlRecovStage() {
...
double LocalBucklingFlangePlate::computeYieldStressTotPlRecovStage() {
	/*double bound4Smoothin = 5. / 100. * abs(SMALL_NUMBER);*/
	double bound4Smoothin = 200. / 100. * abs(SMALL_NUMBER);
...
/* ----------------------------------------------------------------------------------------------------------------- */
double LocalBucklingFlangePlate::computeDAlpha11TotDEpsiPb11PlRecovStage() {
	/*double bound4Smoothin = 5. / 100. * abs(SMALL_NUMBER);*/
	double bound4Smoothin = 200. / 100. * abs(SMALL_NUMBER);
...
/* ----------------------------------------------------------------------------------------------------------------- */
double LocalBucklingFlangePlate::computeDSigmaYieldTotDEpsiPb11PlRecovStage() {
	/*double bound4Smoothin = 5. / 100. * abs(SMALL_NUMBER);*/
	double bound4Smoothin = 200. / 100. * abs(SMALL_NUMBER);
...
/* ----------------------------------------------------------------------------------------------------------------- */
void LocalBucklingFlangePlate::computeReduceC1cLinearEvol() {
	/*double bound4Smoothin = 5. / 100. * abs(SMALL_NUMBER);*/
	double bound4Smoothin = 200. / 100. * abs(SMALL_NUMBER);
...
*
*/
Matrix LocalBucklingFlangePlate::matinv3(const Matrix& A) {
	double detInv;
	Matrix B = Matrix(3, 3);
