...
Matrix FBCElemKNSGINUS2d::theMatrix(6, 6);
Vector FBCElemKNSGINUS2d::theVector(6);
double FBCElemKNSGINUS2d::workArea[200];

Vector FBCElemKNSGINUS2d::eNonLocalSubdivide[maxNumSections];
Vector FBCElemKNSGINUS2d::eLocalSubdivide[maxNumSections];
Matrix FBCElemKNSGINUS2d::FSectionSubdivide[maxNumSections];
Vector FBCElemKNSGINUS2d::srSubdivide[maxNumSections];

// Method to read the command arguments
void* OPS_FBCElemKNSGINUS2d()
{
	...
	Element* theEle = new FBCElemKNSGINUS2d(eleTag, nodeTagI, nodeTagJ, *theCoordTransf, *theBeamIntegration,
...
FBCElemKNSGINUS2d::FBCElemKNSGINUS2d() : Element(0, ELE_TAG_FBCElemKNSGINUS2d), connectedExternalNodes(2), beamIntegr(0), numSections(0), sections(0), crdTransf(0),
...
FBCElemKNSGINUS2d::FBCElemKNSGINUS2d(int tag, int nodeI, int nodeJ, CrdTransf& CT, BeamIntegration& BI,
	SectionForceDeformation** sec, int numSec, int maxNumIters, double tolerance, double LC)
	:Element(tag, ELE_TAG_FBCElemKNSGINUS2d), connectedExternalNodes(2), beamIntegr(0), numSections(0), sections(0), crdTransf(0),
	...
	// complete
{
	...
	if (connectedExternalNodes.Size() != 2) {
		opserr << "WARNING! FBCElemKNSGINUS2d::FBCElemKNSGINUS2d(): " << this->getTag() << " - failed to create an ID of size 2\n";
		exit(-1);
	}
	...
	if (!beamIntegr) {
		opserr << "WARNING! FBCElemKNSGINUS2d::FBCElemKNSGINUS2d(): " << this->getTag() << " - could not create copy of beam integration object" << endln;
		exit(-1);
	}
	...
	if (crdTransf == 0) {
		opserr << "WARNING! FBCElemKNSGINUS2d::FBCElemKNSGINUS2d(): could not create copy of coordinate transformation object" << endln;
		exit(-1);
	}
...
FBCElemKNSGINUS2d::~FBCElemKNSGINUS2d()
{
	if (sections != 0) {
...
int
FBCElemKNSGINUS2d::getNumExternalNodes(void) const
{
	return 2;
}

const ID&
FBCElemKNSGINUS2d::getExternalNodes(void)
{
	return connectedExternalNodes;
}

Node**
FBCElemKNSGINUS2d::getNodePtrs()
{
	return theNodes;
}

int
FBCElemKNSGINUS2d::getNumDOF(void)
{
	return NEGD;
}
...
void
FBCElemKNSGINUS2d::setDomain(Domain* theDomain)
{
	...
	if (theDomain == 0) {
		...
		opserr << "ERROR! FBCElemKNSGINUS2d::setDomain():  theDomain = 0 ";
		exit(0);
	}
	...
	if (theNodes[0] == 0) {
		opserr << "ERROR! FBCElemKNSGINUS2d::setDomain: Nd1: ";
		opserr << Nd1 << "does not exist in model\n";
		exit(0);
	}

	if (theNodes[1] == 0) {
		opserr << "ERROR! FBCElemKNSGINUS2d::setDomain: Nd2: ";
	...
	if (dofNode1 != 3) {
		opserr << "ERROR! FBCElemKNSGINUS2d::setDomain() - element: " << this->getTag() << " - node " << Nd1 << " does not have 3 DOFs\n";
		exit(0);
	}

	if (dofNode2 != 3) {
		opserr << "ERROR! FBCElemKNSGINUS2d::setDomain() - element: " << this->getTag() << " - node " << Nd2 << " does not have 3 DOFs\n";
		exit(0);
	}

	// Initialize Coordinate Transformation
	if (crdTransf->initialize(theNodes[0], theNodes[1])) {
		opserr << "WARNING! FBCElemKNSGINUS2d::setDomain() - element: " << this->getTag() << " - Error initializing coordinate transformation\n";
		exit(0);
	}
	...
	if (L == 0.0) {
		opserr << "WARNING! FBCElemKNSGINUS2d::setDomain(): element has zero length:" << this->getTag();
		exit(0);
	}
...
int
FBCElemKNSGINUS2d::commitState()
{
	int err = 0;

	// Element commitState()
	if ((err = this->Element::commitState()))
		opserr << "WARNING! FBCElemKNSGINUS2d::commitState() - element: " << this->getTag() << " - failed in committing base class\n";
	...
	if ((err = crdTransf->commitState()) != 0)
		opserr << "WARNING! FBCElemKNSGINUS2d::commitState() - element: " << this->getTag() << " - failed to commit coordinate transformation object\n";
...
int
FBCElemKNSGINUS2d::revertToLastCommit(void)
{
	int err = 0;
	...
	if ((err = crdTransf->revertToLastCommit()))
		opserr << "WARNING! FBCElemKNSGINUS2d::revertToLastCommit() - element: " << this->getTag() << " - coordinate transformation object failed to revert to last committed state\n";
...
int
FBCElemKNSGINUS2d::revertToStart(void)
{
	...
	if ((err = crdTransf->revertToStart()) != 0)
		opserr << "WARNING! FBCElemKNSGINUS2d::revertToStart() - element: " << this->getTag() << " - failed to revert to start coordinate transformation object\n";
...
const Matrix&
FBCElemKNSGINUS2d::getInitialStiff(void)
{
...
const Matrix&
FBCElemKNSGINUS2d::getTangentStiff(void)
{
	crdTransf->update();
...
void
FBCElemKNSGINUS2d::computeReactions(double* p0)
{
	int type;
	double L = crdTransf->getInitialLength();
...
const Vector&
FBCElemKNSGINUS2d::getResistingForce(void)
{
	double p0[3];
	Vector p0Vec(p0, 3);
	p0Vec.Zero();
...
void
FBCElemKNSGINUS2d::initializeSectionHistoryVariables(void)
{
	for (int i = 0; i < numSections; i++) {
...
FBCElemKNSGINUS2d::update(void)
{
			...
			if (initialFlag != 2)
			{
				...
				for (j = 0; j < numItersMax; j++)
				{
					...
					for (i = 0; i < numSections; i++)
					{
						...
						{
							opserr << "FBCElemKNSGINUS2d::update() - section failed in setTrial\n";
					...
					if (Felement.Solve(I, KelementTrial) < 0) {
						opserr << "FBCElemKNSGINUS2d::update() -- could not invert flexibility\n";
	...
	if (converged == false)
	{
		opserr << "WARNING - FBCElemKNSGINUS2d::update - failed to get compatible ";
...
void FBCElemKNSGINUS2d::getForceInterpolatMatrix(double xi, Matrix& b, const ID& code)
{
	b.Zero();
...
void FBCElemKNSGINUS2d::getDistrLoadInterpolatMatrix(double xi, Matrix& bp, const ID& code)
{
	bp.Zero();
...
void
FBCElemKNSGINUS2d::computeSectionForces(Vector& sp, int isec)
{
	int type;
	...
	for (int i = 0; i < numEleLoads; i++)
	{
		...
		{
			opserr << "FBCElemKNSGINUS2d::addLoad -- load type unknown for element with tag: " <<
				this->getTag() << endln;
		}
	}
}
...
const Matrix&
FBCElemKNSGINUS2d::getMass(void)
{
	theMatrix.Zero();
...
void
FBCElemKNSGINUS2d::zeroLoad(void)
{
	load.Zero();
...
int
FBCElemKNSGINUS2d::addLoad(ElementalLoad* theLoad, double loadFactor)
{
...
const Vector&
FBCElemKNSGINUS2d::getResistingForceIncInertia()
{
...
int
FBCElemKNSGINUS2d::sendSelf(int commitTag, Channel& theChannel)
{
	// No parallel processing for now
	opserr << "WARNING! FBCElemKNSGINUS2d::sendSelf() - element: " << this->getTag() << " - no parallel processing for now\n";
	return -1;
}

//Method for receiving itself for parallel processing
int
FBCElemKNSGINUS2d::recvSelf(int commitTag, Channel& theChannel, FEM_ObjectBroker& theBroker)
{
	// No parallel processing for now
	opserr << "WARNING! FBCElemKNSGINUS2d::recvSelf() - element: " << this->getTag() << " - no parallel processing for now\n";
	return -1;
}

//Method for computing initial element flexibility matrix
int
FBCElemKNSGINUS2d::getInitialFlexibility(Matrix& Fe)
{
	Fe.Zero();
...
//Method for printing 
void
FBCElemKNSGINUS2d::Print(OPS_Stream& s, int flag)
{
	s << "Element Tag: " << this->getTag() << endln;
	s << "Type: FBCElemKNSGINUS2d" << endln;
...
int
FBCElemKNSGINUS2d::displaySelf(Renderer& theViewer, int displayMode, float fact, const char** displayModes, int numModes)
{
	static Vector v1(3);
	static Vector v2(3);
...
Response*
FBCElemKNSGINUS2d::setResponse(const char** argv, int argc, OPS_Stream& output)
{
...
int
FBCElemKNSGINUS2d::getResponse(int responseID, Information& eleInfo)
{
	switch (responseID)
	{
...
void
FBCElemKNSGINUS2d::setSectionPointers(int numSec, SectionForceDeformation** secPtrs)
{
	...
	if (secPtrs == 0) {
		opserr << "Error: FBCElemKNSGINUS2d::setSectionPointers -- invalid section pointer";
	}

	sections = new SectionForceDeformation * [numSections];
	if (sections == 0) {
		opserr << "Error: FBCElemKNSGINUS2d::setSectionPointers -- could not allocate section pointers";
	}

	for (int i = 0; i < numSections; i++) {

		if (secPtrs[i] == 0) {
			opserr << "Error: FBCElemKNSGINUS2d::setSectionPointers -- null section pointer " << i << endln;
		}

		sections[i] = secPtrs[i]->getCopy();

		if (sections[i] == 0) {
			opserr << "Error: FBCElemKNSGINUS2d::setSectionPointers -- could not create copy of section " << i << endln;
		}
	}
...
void
FBCElemKNSGINUS2d::initCoefficientMatrixH()
{
	double L = crdTransf->getInitialLength();
...
void
FBCElemKNSGINUS2d::computeCoefficientMatrixH()
{
...
void
FBCElemKNSGINUS2d::computeMatrixH()
{
...
void
FBCElemKNSGINUS2d::computeMatrixH_inv()
{
	H_inv.Zero();
	if (H.Invert(H_inv) < 0)
		opserr << "FBCElemKNSGINUS2d::update() -- could not invert matrix H\n";
}

//Method to compute deStar_nonlocal[]
void
FBCElemKNSGINUS2d::computeDeStar_nonlocal(Vector deStar_nonlocal_Tot[], Vector deStar_local_Tot[])
{
...
void
FBCElemKNSGINUS2d::computeE_local(Matrix& e_local_tot)
{
	e_local_tot.Zero();
...
void
FBCElemKNSGINUS2d::computeEu_nonlocal(Vector eu_nonlocal_Tot[], Vector eu_local_Tot[])
{
	//eu_nonlocal.Zero();
...
void
FBCElemKNSGINUS2d::computeFelement_nonlocal(Matrix& Felement_nonlocal)
{
