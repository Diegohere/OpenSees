//
// Created by Diego Heredia on 16.07.25
// Contains the source code for the Tcl commands of Force Beam-Column Elem Krylov-Newton Selective Gradient Inelasticity Non Uniform Spacing
...
int
TclModelBuilder_addFBCElemKNSGINUS(ClientData clientData, Tcl_Interp* interp,
	int inArgc,
	TCL_Char** inArgv,
	Domain* theTclDomain,
	TclModelBuilder* theTclBuilder)
{
	...
	if (ndm == 2) {
		theElement = new FBCElemKNSGINUS2d(eleTag, iNode, jNode, *theCoordTransf2d, *beamIntegr, IntegrSections, numIntegrPts, maxNumIter, tolerance, lc);
	}
	else if (ndm == 3) {
		theElement = new FBCElemKNSGINUS3d(eleTag, iNode, jNode, *theCoordTransf3d, *beamIntegr, IntegrSections, numIntegrPts, maxNumIter, tolerance, lc);
	}
