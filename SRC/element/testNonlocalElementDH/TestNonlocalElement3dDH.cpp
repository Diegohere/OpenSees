		...
		for (int l = 0; l < 3; l++)
		{
			...
			if (initialFlag != 2)
			{
				...
				for (j = 0; j < numIters; j++)
				{
					...
					d2ThetaZDX2(0) = (eNonLocalSubdivide[1](1) - eNonLocalSubdivide[0](1)) / ((xi[1] - xi[0]) * L);
					d2ThetaYDX2(0) = (eNonLocalSubdivide[1](2) - eNonLocalSubdivide[0](2)) / ((xi[1] - xi[0]) * L);
					for (int i = 1; i < numSections-1; i++)
					{
						d2ThetaZDX2(i) = (eNonLocalSubdivide[i + 1](1) - eNonLocalSubdivide[i - 1](1)) / (2 * (xi[i + 1] - xi[i - 1]) * L);
						d2ThetaYDX2(i) = (eNonLocalSubdivide[i + 1](2) - eNonLocalSubdivide[i - 1](2)) / (2 * (xi[i + 1] - xi[i - 1]) * L);
					}
					d2ThetaZDX2(numSections - 1) = (eNonLocalSubdivide[numSections - 1](1) - eNonLocalSubdivide[numSections - 2](1)) / ((xi[numSections - 1] - xi[numSections - 2]) * L);
					d2ThetaYDX2(numSections - 1) = (eNonLocalSubdivide[numSections - 1](2) - eNonLocalSubdivide[numSections - 2](2)) / ((xi[numSections - 1] - xi[numSections - 2]) * L);
					...
					for (i = 0; i < numSections; i++)
					{
						...
						if (sections[i]->setTrialSectionDeformation(eLocalSubdivide[i], d2ThetaZDX2(i), d2ThetaYDX2(i)) < 0)
						{
